...
// Initialize the kernel event queue
void
kqueueinit(void)
{
...
kqueue_post(const char *event_name, int pid)
{
  ...
  // If queue is full, discard oldest event
  if(kq.count >= NEVENT) {
  ...
  e->timestamp = r_time();  // Get current kernel ticks
...
// Syscall wrapper for kqueue_post from user space
uint64
sys_kqueue_post(void)
{
  int pid;
  char name[MAXEVENTNAME];
  
  argint(0, &pid);
  argstr(1, name, MAXEVENTNAME);
  
  kqueue_post(name, pid);
  return 0;
}

// Wait for an event from the kernel queue (kernel version)
// Returns event in user-supplied buffer
void
kqueue_wait_internal(struct event *ev)
{
  ...
  struct event *e = &kq.events[kq.head];
  *ev = *e;
  kq.head = (kq.head + 1) % NEVENT;
  kq.count--;
...
// Syscall wrapper for kqueue_wait
uint64
sys_kqueue_wait(void)
{
  ...
  kqueue_wait_internal(&ev);
  
  if(copyout(myproc()->pagetable, evp, (char *)&ev, sizeof(ev)) < 0)
    return -1;
